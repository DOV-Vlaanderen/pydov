...
class GrondwaterLocatieSearch(AbstractSearch):
    ...
    def __init__(self, objecttype=GrondwaterLocatie):
        """Initialisation.
        ...
        """
        super(GrondwaterLocatieSearch,
              self).__init__('gw_meetnetten:meetnetten', objecttype)

    def _init_namespace(self):
        """Initialise the WFS namespace associated with the layer."""
        if GrondwaterLocatieSearch.__wfs_namespace is None:
            GrondwaterLocatieSearch.__wfs_namespace = self._get_namespace()

    def _init_fields(self):
        """Initialise the fields and their metadata available in this search
        class."""
        if self._fields is None:
            if GrondwaterLocatieSearch.__wfs_schema is None:
                GrondwaterLocatieSearch.__wfs_schema = self._get_schema()

            if GrondwaterLocatieSearch.__md_metadata is None:
                GrondwaterLocatieSearch.__md_metadata = \
                    self._get_remote_metadata()

            if GrondwaterLocatieSearch.__fc_featurecatalogue is None:
                csw_url = self._get_csw_base_url()
                fc_uuid = owsutil.get_featurecatalogue_uuid(
                    GrondwaterLocatieSearch.__md_metadata)

                GrondwaterLocatieSearch.__fc_featurecatalogue = \
                    owsutil.get_remote_featurecatalogue(csw_url, fc_uuid)

            if GrondwaterLocatieSearch.__xsd_schemas is None:
                GrondwaterLocatieSearch.__xsd_schemas = \
                    self._get_remote_xsd_schemas()

            fields = self._build_fields(
                GrondwaterLocatieSearch.__wfs_schema,
                GrondwaterLocatieSearch.__fc_featurecatalogue,
                GrondwaterLocatieSearch.__xsd_schemas)
    ...
    def search(self, location=None, query=None, return_fields=None,
        ...
        gwlocaties = self._type.from_wfs(fts, self.__wfs_namespace)

        df = pd.DataFrame(
            data=self._type.to_df_array(gwlocaties, return_fields),
